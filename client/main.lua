...
CreateThread(function()
    if GetResourceState('karma-dispatch') == 'started' then
        TriggerServerEvent("karma-mdt:dispatchStatus", true)
    end
end)
...
RegisterNUICallback("removeCallBlip", function(data, cb)
    TriggerEvent('karma-dispatch:client:removeCallBlip', data.callid or data.id)
    cb(true)
end)
...
RegisterNetEvent('karma-mdt:client:selfregister')
AddEventHandler('karma-mdt:client:selfregister', function()
    ...
    AddEventHandler("karma-mdt:client:trafficStop", function()
        ...
        if not coolDown then
            if data.veh ~= nil and data.veh ~= 0 then
                ...
                exports["karma-dispatch"]:CustomAlert({
                    coords = {
                        x = currentPos.x,
                        y = currentPos.y,
                        z = currentPos.z
