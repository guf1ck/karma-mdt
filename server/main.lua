...
QBCore.Functions.CreateCallback('karma-mdt:server:MugShotWebhook', function(source, cb)
...
RegisterServerEvent("karma-mdt:dispatchStatus", function(bool)
...
AddEventHandler('onResourceStart', function(resourceName)
	...
	-- if GetResourceState('karma-dispatch') == 'started' then
	-- 	local calls = exports['karma-dispatch']:GetDispatchCalls()
	-- 	return calls
	-- end
end)

RegisterNetEvent("karma-mdt:server:OnPlayerUnload", function()
...
RegisterNetEvent("karma-mdt:server:ToggleDuty", function()
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
...
QBCore.Commands.Add("mdtleaderboard", "Show MDT leaderboard", {}, false, function(source, args)
    ...
    sendToDiscord(16753920, "MDT Leaderboard", leaderboard_message, "karma-mdt | Made by Project Sloth")
    TriggerClientEvent('QBCore:Notify', source, "MDT leaderboard sent to Discord!", 'success')
end)

RegisterNetEvent("karma-mdt:server:ClockSystem", function()
    local src = source
    local PlayerData = GetPlayerData(src)
    local time = os.date("%Y-%m-%d %H:%M:%S")
    local firstName = PlayerData.charinfo.firstname:sub(1,1):upper()..PlayerData.charinfo.firstname:sub(2)
...
RegisterNetEvent('mdt:server:openMDT', function()
	...
	-- if GetResourceState('karma-dispatch') == 'started' then
	-- 	calls = exports['karma-dispatch']:GetDispatchCalls()
	-- end
...
QBCore.Functions.CreateCallback('karma-mdt:getDispatchCalls', function(source, cb)
    local calls = exports['karma-dispatch']:GetDispatchCalls()
    cb(calls)
end)
	...
	if Player then
		...
		if JobType == 'police' or JobType == 'doj' then
			...
			for _, value in ipairs(vehicles) do
				...
				value.code = false
				value.stolen = false
				value.image = "images/not-found.webp"
		...
		if Player then
			...
			if JobType == 'police' or JobType == 'doj' then
				...
				if vehicle and vehicle[1] then
					...
					if vehicle[1]['image'] == nil then vehicle[1]['image'] = "images/not-found.webp" end
				end
					...
					if impoundInfo.impoundActive then
						...
						if vehicle.impoundid ~= nil then
							...
							if result then
								...
								result.plate = plate
								TriggerClientEvent('karma-mdt:client:TakeOutImpound', src, result)
							end
...
RegisterNetEvent('mdt:server:saveWeaponInfo', function(serial, imageurl, notes, owner, weapClass, weapModel)
	if serial then
		...
		if Player then
			...
			if JobType == 'police' or JobType == 'doj' then
				local fullname = Player.PlayerData.charinfo.firstname .. ' ' .. Player.PlayerData.charinfo.lastname
				if imageurl == nil then imageurl = 'images/not-found.webp' end
				--AddLog event?
				local result = false
                        ...
                        MySQL.Async.fetchAll('SELECT `author`, `title`, `details` FROM `mdt_incidents` WHERE `id` = @id', { ['@id'] = infoResult }, function(result)
                            if result and #result > 0 then
                                ...
                                for i=1, #associated do
                                    ...
                                    sendIncidentToDiscord(3989503, "MDT Incident Report", message, "karma-mdt | Made by Project Sloth", associatedData)                                
                                end
                            else
                ...
                MySQL.Async.fetchAll('SELECT `author`, `title`, `details` FROM `mdt_incidents` WHERE `id` = @id', { ['@id'] = id }, function(result)
                    if result and #result > 0 then
                        ...
                        for i=1, #associated do
                            ...
                            sendIncidentToDiscord(16711680, "MDT Incident Report has been Updated", message, "karma-mdt | Made by Project Sloth", associatedData)
                        end
                    else
...
RegisterNetEvent('mdt:server:registerweapon', function(serial, imageurl, notes, owner, weapClass, weapModel) 
    exports['karma-mdt']:CreateWeaponInfo(serial, imageurl, notes, owner, weapClass, weapModel)
end)
